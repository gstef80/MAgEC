...
def clean_demo_df(demo, hours, drop=True):
    """
    return patients who stayed in ICU at least that many hours
    """
    # no need to yell 
...
filtered_labs_template = \
"""
WITH pvt AS (
  SELECT ie.subject_id, ie.hadm_id, ie.icustay_id, le.charttime, mv.mv_start, mv.ventnum
  , CASE
  ...
  LEFT JOIN labevents le
    ...
    ON le.subject_id = mv.subject_id
    AND le.hadm_id = mv.hadm_id
),
ranked AS (
...
SELECT r.subject_id, r.hadm_id, r.icustay_id, 
    ...
    date(min(r.mv_start)) as mv_start,
    min(r.ventnum) as ventnum
...
def mv_labs():
    return filtered_labs_template


filtered_vits_template = \
"""
SELECT pvt.subject_id, pvt.hadm_id, pvt.icustay_id, pvt.lab_date
, date(min(pvt.mv_start)) as mv_start
, min(pvt.ventnum) as ventnum
...
FROM  (
  select ie.subject_id, ie.hadm_id, ie.icustay_id, date(ce.charttime) as lab_date, mv.mv_start, mv.ventnum
  , case
